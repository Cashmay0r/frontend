    ...
    <div class="h-auto flex flex-col justify-center items-center mt-10 w-4/6 m-auto text-gray-text">
      <form @submit.prevent="login" class="w-full" v-if="!resettingPassword">
        ...
        <div class="flex flex-col justify-start items-start w-full">
          ...
          <input type="email" class="input input-bordered w-full text-sm bg-white" placeholder="Enter Email" name="email" v-model="user.email" required autofocus />
        </div>
        <div class="flex flex-col justify-center items-start w-full">
          <label for="password" class="label text-sm">Password</label>
          <input type="password" class="input input-bordered w-full text-sm bg-white" placeholder="Enter Password" name="password" v-model="user.password" required />
          <p v-text="errorMsg.message" class="text-red-700 text-sm label" v-if="errorMsg.status"></p>
          <p v-text="successMsg.message" class="text-primary text-sm label" v-if="successMsg.status"></p>
        </div>
        <div class="w-full flex flex-row justify-end items-center gap-2">
          <a class="underline label text-sm text-primary" @click="resettingPassword = true">Forgot password?</a>
        </div>
      ...
      <form @submit.prevent="resetPassword" v-if="resettingPassword" class="w-full">
        ...
        <div class="flex flex-col justify-start items-start w-full">
          <label for="email" class="label text-sm">Email:</label>
          <input type="email" class="input input-bordered w-full text-sm bg-white" placeholder="Enter Email" name="email" v-model="resetEmail" />
        </div>
        <div class="flex flex-col justify-center items-center w-full mt-5">
          <button class="btn w-1/2 bg-primary text-white" type="submit">Send</button>
        </div>
      </form>
      <div class="w-full flex flex-row justify-end items-center" v-if="!resettingPassword">
  ...
  const login = () => {
    ...
    signInWithEmailAndPassword(auth, user.email, user.password)
      ...
      .catch((error) => {
        ...
        if (error.code == 'auth/too-many-requests') errorMsg.message = 'Too many login attempts, please reset password.';
